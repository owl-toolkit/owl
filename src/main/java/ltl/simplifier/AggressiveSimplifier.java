    ...
    for (Formula form : set) {
      for (Formula form2 : set) {
        if (!form.equals(form2)) {
          ...
          PseudoSubstitutionVisitor visitor = new PseudoSubstitutionVisitor(form2,
    ...
    if (newF instanceof FOperator) {
      ...
      if (child instanceof Disjunction) {
        return (new Disjunction(
          ((PropositionalFormula) child).children.stream().map(FOperator::new))).accept(this);
      }
    }
    return newF;
  }
    ...
    if (newG instanceof GOperator) {
      ...
      if (child instanceof Conjunction) {
        return (new Conjunction(
          ((PropositionalFormula) child).children.stream().map(GOperator::new))).accept(this);
      }

      if (child instanceof UOperator) {
        Formula l = new GOperator(
          new Disjunction(((UOperator) child).left, ((UOperator) child).right));
  ...
  public Formula visit(ROperator r) {
    ...
    if (newFormula instanceof ROperator) {
      ...
      if (newR.left.isPureEventual()) {
        return Disjunction
          .create(Conjunction.create(newR.left, newR.right), GOperator.create(newR.right));
      }
      return ROperator
        .create(newR.left.accept(new PseudoSubstitutionVisitor(newR.right, BooleanConstant.TRUE)),
          newR.right);
    }
    return newFormula;
  }
    ...
    if (newU instanceof UOperator) {
      ...
      if (l instanceof XOperator && r instanceof XOperator) {
        return new XOperator(
          new UOperator(((UnaryModalOperator) l).operand, ((UnaryModalOperator) r).operand));
      }

      if (l instanceof Conjunction) {
        return Simplifier.simplify(new Conjunction(
            ((Conjunction) l).children.stream().map(left -> new UOperator(left, r))
              .collect(Collectors.toSet())),
          Simplifier.Strategy.AGGRESSIVELY);
      }

      if (r instanceof Disjunction) {
        return Simplifier.simplify(new Disjunction(
            ((Disjunction) r).children.stream().map(right -> new UOperator(l, right))
              .collect(Collectors.toSet())),
          Simplifier.Strategy.AGGRESSIVELY);
      }
    }
