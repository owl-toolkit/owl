...
public final class LtlfToLtlTranslator {
  ...
  public static class LtlfToLtlVisitor implements Visitor<Formula> {
    ...
    public static class PushNegOneDownVisitor implements Visitor<Formula> {
      ...
      @Override
      public Formula visit(Biconditional biconditional) {
        return new Biconditional(biconditional.left, new Negation(biconditional.right));
      }
      ...
      public Formula visit(Conjunction conjunction) {
        ...
        return new Disjunction(((Stream<? extends Formula>) conjunction.children
          .stream()
      ...
      public Formula visit(Disjunction disjunction) {
        ...
        return new Conjunction(((Stream<? extends Formula>) disjunction.children
          .stream()
          .map(Negation::new)).collect(Collectors.toUnmodifiableSet()));
      }
      ...
      @Override
      public Formula visit(MOperator mOperator) {
        return new WOperator(new Negation(mOperator.left), new Negation(mOperator.right));
      }

      @Override
      public Formula visit(ROperator rOperator) {
        return new UOperator(new Negation(rOperator.left), new Negation(rOperator.right));
      }

      @Override
      public Formula visit(UOperator uOperator) {
        return new ROperator(new Negation(uOperator.left), new Negation(uOperator.right));
      }

      @Override
      public Formula visit(WOperator wOperator) {
        return new MOperator(new Negation(wOperator.left), new Negation(wOperator.right));
      }
