 ...
 * given by either an {@link Automaton} or a {@link EdgeRelation}.
 *
...
public abstract class SccDecomposition<S> {
  ...
  protected abstract EdgeRelation<S> successorFunction();
  ...
  public static <S> SccDecomposition<S> of(Automaton<S, ?> automaton) {
    return new AutoValue_SccDecomposition<>(
        Set.copyOf(automaton.initialStates()), automaton::edges, automaton);
  }

  public static <S> SccDecomposition<S> of(
      Set<? extends S> initialStates, EdgeRelation<S> successorsFunction) {
    return new AutoValue_SccDecomposition<>(Set.copyOf(initialStates), successorsFunction, null);
  }
   ...
   * the result. If the initial states are empty then {@code false} is returned and the predicate is
   * not evaluated.
   *
   ...
   * evaluate the predicate on all strongly connected components if not necessary for determining
   * the result. If the initial states are empty then {@code true} is returned and the predicate is
   * not evaluated.
   *
   ...
   * are vertices, ordered such that for each transition {@code a->b} in the condensation graph, a
  ...
  public List<Set<S>> sccs() {
    ...
    class MonitoringEdgeRelation implements EdgeRelation<S> {
      ...
      public Set<S> successors(S state) {
        var successors = successorFunction.successors(state);
    ...
    var tarjan = new Tarjan<>(new MonitoringEdgeRelation(), x -> {
   ...
   * {@code -1} if no such {@code i} exists (only if {@code state} is not part of the automaton)
   */
  ...
  public Map<S, Integer> indexMap() {
    var sccs = sccs();
    int states = sccs.stream().mapToInt(Set::size).sum();

    @SuppressWarnings("unchecked")
    Map.Entry<S, Integer>[] indexMap = new Map.Entry[states];
    int j = 0;
    ...
    for (int i = 0, s = sccs.size(); i < s; i++) {
      ...
      for (S state : sccs.get(i)) {
        indexMap[j] = Map.entry(state, iObject);
        j++;
      }
    }

    return Map.ofEntries(indexMap);
  }

  /**
   * Compute the condensation graph corresponding to the SCC decomposition. The {@code Integer}
   * vertices correspond to the index in the list returned by {@link SccDecomposition#sccs}. Every
   * path in this graph is labelled by monotonic increasing ids.
   *
  ...
  public ImmutableGraph<Integer> condensation() {
    ...
    for (Set<S> scc : sccs()) {
      ...
      for (S state : scc) {
        for (S successor : successorFunction().successors(state)) {
  ...
  /**
   * Return indices of all strongly connected components that are bottom. A SCC is considered bottom
   * if it is not transient and there are no transitions leaving it.
   *
  ...
  /**
   * Determine if a given strongly connected component is bottom. A SCC is considered bottom if
   * there are no transitions leaving it.
   *
  ...
  public ImmutableBitSet acceptingSccs() {
    ...
    var acceptance = (BuchiAcceptance) automaton().acceptance();
    ...
    for (int i = 0, s = sccs().size(); i < s; i++) {
      ...
      Views.Filter<S> justRej = Views.Filter.<S>builder()
          .initialStates(scc)
          .edgeFilter((state, edge) ->
              scc.contains(edge.successor()) && !acceptance.isAcceptingEdge(edge))
          .build();
  ...
  public ImmutableBitSet rejectingSccs() {
    ...
    for (int i = 0, s = sccs().size(); i < s; i++) {
      var scc = sccs.get(i);
      rejectingSccs.set(i, LanguageEmptiness.isEmpty(
          Set.of(scc.iterator().next()),
          EdgeRelation.filter(automaton::edges, edge -> scc.contains(edge.successor())),
